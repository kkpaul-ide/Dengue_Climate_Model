...
den_model <- function(project, params, data, initial_conditions, option = "difference") {
  ...
  if (option == "difference") {
    ...
    muv <- 1 - exp(-muv_th(temp,hum,1) * dt)
    ...
    for (tt in 2:time) {
      ...
      s_l[tt] <- s_l[tt-1] + (EFD[tt]*FEL[tt]*max((1-Nl[tt-1]/(Nh[tt-1]*cf*C_t(rain[tt]))),0)*0.5*Nv[tt-1])*(1 - params$nu*(i_v1[tt-1] + i_v2[tt-1])/Nv[tt-1]) - 2*LMR[tt]*s_l[tt-1] -  mul_tr[tt]*s_l[tt-1] 
      i_l1[tt] <- i_l1[tt-1] + (EFD[tt]*FEL[tt]*max((1-Nl[tt-1]/(Nh[tt-1]*cf*C_t(rain[tt]))),0)*0.5*Nv[tt-1])*params$nu*i_v1[tt-1]/Nv[tt-1] - LMR[tt]*i_l1[tt-1] - mul_tr[tt]*i_l1[tt-1]
      i_l2[tt] <- i_l2[tt-1] + (EFD[tt]*FEL[tt]*max((1-Nl[tt-1]/(Nh[tt-1]*cf*C_t(rain[tt]))),0)*0.5*Nv[tt-1])*params$nu*i_v2[tt-1]/Nv[tt-1] - LMR[tt]*i_l2[tt-1] - mul_tr[tt]*i_l2[tt-1]
      ...
      s_h[tt] <- s_h[tt-1] +  BR[tt]*Nh[tt-1] - # newpop[tt] -
        a[tt]*pIM[tt]*((i_v1[tt-1] + i_v2[tt-1])/Nh[tt-1])*s_h[tt-1] - DR[tt]*s_h[tt-1]  + im[tt]*Nh[tt-1]*(1-params$exseroprev*param_set$serofactor) - em[tt]*s_h[tt-1] 
      e_h1[tt] <- e_h1[tt-1] + a[tt]*pIM[tt]*(i_v1[tt-1]/Nh[tt-1])*s_h[tt-1] - params$alpha1*e_h1[tt-1] - DR[tt]*e_h1[tt-1] + influx1[tt-1] - em[tt]*e_h1[tt-1] 
      ...
      r_h1[tt] <- r_h1[tt-1] + params$sigma1*ci_h1[tt-1] - a[tt]*pIM[tt]*(i_v2[tt-1]/Nh[tt-1])*r_h1[tt-1] - DR[tt]*r_h1[tt-1] + im[tt]*Nh[tt-1]*params$exseroprev*param_set$serofactor*(r_h1[tt-1]/(r_h1[tt-1]+r_h2[tt-1]+r[tt-1])) - em[tt]*r_h1[tt-1]
      r_h2[tt] <- r_h2[tt-1] + params$sigma2*ci_h2[tt-1] - a[tt]*pIM[tt]*(i_v1[tt-1]/Nh[tt-1])*r_h2[tt-1] - DR[tt]*r_h2[tt-1] + im[tt]*Nh[tt-1]*params$exseroprev*param_set$serofactor*(r_h2[tt-1]/(r_h1[tt-1]+r_h2[tt-1]+r[tt-1])) - em[tt]*r_h2[tt-1]
      ...
      r[tt] <- r[tt-1] + params$gamma2*i_h12[tt-1] + params$gamma1*i_h21[tt-1] - DR[tt]*r[tt-1] + im[tt]*Nh[tt-1]*params$exseroprev*param_set$serofactor*(r[tt-1]/(r_h1[tt-1]+r_h2[tt-1]+r[tt-1])) - em[tt]*r[tt-1]
