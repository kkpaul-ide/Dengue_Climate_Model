...
for (source in gcmOptions) {
  ...
  for (scenario in rcpOptions) {
    ...
    climData <- bind_rows(climData, tempData)
    
    # Reorder climate data to have the years in random
    set.seed(sim_seed)
    tempClimData <- list()
    randomYears <- list()
    # for each GCM and RCP, making 10 different sets
    for (set in 1:10){ 
      randomYears[[set]] <- sample(startYear:endYear)
      a <- list()
      for (years in randomYears[[set]]){
        #filter data for randomly selected year
        b <- filter(climData, year==years)
        a <- bind_rows(a,b)
      }
      a$set <- set
      tempClimData <- bind_rows(tempClimData,a)
    }
    climateData <- bind_rows(climateData,tempClimData)
  }
}
